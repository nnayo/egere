...
// START
// write register address to MPU 
// then write as many bytes as possible
// write address is incremented automatically
// STOP
//
...
// read address is incremented automatically
// NACK
// STOP
//
...
static PT_THREAD( mpu_init_pt_thread(pt_t* pt) )
{
...
#ifndef USE_SC18IS600
        PT_WAIT_UNTIL(pt, scalp_set_1(&fr, MPU_I2C_ADDR, SCALP_DPT_SELF_ADDR, SCALP_TWIWRITE, MPU6050_WHO_AM_I)
    ...
    PT_SPAWN(pt, &mpu.pt_spawn_2, SC18IS600_tx(&mpu.pt_spawn_2, MPU_I2C_ADDR, tx, &mpu.n));
        ...
        // read WHO_AM_I reg
#ifndef USE_SC18IS600
        PT_WAIT_UNTIL(pt, scalp_set_1(&fr, MPU_I2C_ADDR, SCALP_DPT_SELF_ADDR, SCALP_TWIREAD, 0)
                        && scalp_dpt_tx(&mpu.interf, &fr));
        // wait response
        PT_WAIT_UNTIL(pt, OK == nnk_fifo_get(&mpu.in_fifo, &fr));
#else
...
#ifndef USE_SC18IS600
        PT_WAIT_UNTIL(pt, scalp_set_5(&fr, MPU_I2C_ADDR, SCALP_DPT_SELF_ADDR, SCALP_TWIWRITE, MPU6050_SMPLRT_DIV, 0x09, 0x02, 0x08, 0x18)
                        && scalp_dpt_tx(&mpu.interf, &fr));
        // wait response
        PT_WAIT_UNTIL(pt, OK == nnk_fifo_get(&mpu.in_fifo, &fr));
#else
...
#ifndef USE_SC18IS600
        PT_WAIT_UNTIL(pt, scalp_set_2(&fr, MPU_I2C_ADDR, SCALP_DPT_SELF_ADDR, SCALP_TWIWRITE, MPU6050_PWR_MGMT_1, 0x00)
                        && scalp_dpt_tx(&mpu.interf, &fr));
        // wait response
        PT_WAIT_UNTIL(pt, OK == nnk_fifo_get(&mpu.in_fifo, &fr));
...
#ifndef USE_SC18IS600
        PT_WAIT_UNTIL(pt, scalp_set_0(fr, MPU_I2C_ADDR, SCALP_DPT_SELF_ADDR, SCALP_TWIREAD)
                        && (fr->len = len)
                        && scalp_dpt_tx(&mpu.interf, fr));
        // wait response
        PT_WAIT_UNTIL(pt, OK == nnk_fifo_get(&mpu.in_fifo, fr));
...
static PT_THREAD( mpu_thread(pt_t* pt) )
{
        ...
        // signal the MPU is ready
        scalp_dpt_lock(&mpu.interf);
        PT_WAIT_UNTIL(pt, scalp_set_1(&fr, SCALP_DPT_SELF_ADDR, SCALP_DPT_SELF_ADDR, SCALP_MINUTEVENT, SCALP_MINU_EV_MPU_READY)
                          && scalp_dpt_tx(&mpu.interf, &fr));
        scalp_dpt_unlock(&mpu.interf);

        // wait a full second before starting the acquisitions
        ...
        while (1) {
...
#ifndef USE_SC18IS600
                PT_WAIT_UNTIL(pt, scalp_set_1(&fr, MPU_I2C_ADDR, SCALP_DPT_SELF_ADDR, SCALP_TWIWRITE, MPU6050_ACCEL_XOUT_H)
                                && scalp_dpt_tx(&mpu.interf, &fr));
                // wait response
                PT_WAIT_UNTIL(pt, OK == nnk_fifo_get(&mpu.in_fifo, &fr));
#else
                ...
                PT_WAIT_UNTIL(pt, scalp_set_6(&fr, SCALP_DPT_SELF_ADDR, SCALP_DPT_SELF_ADDR, SCALP_MPUACC, mpu.data.acc_x_hi, mpu.data.acc_x_lo, mpu.data.acc_y_hi, mpu.data.acc_y_lo, mpu.data.acc_z_hi, mpu.data.acc_z_lo)
                                && scalp_dpt_tx(&mpu.interf, &fr));

                // build and send the gyroscopic data
                PT_WAIT_UNTIL(pt, scalp_set_6(&fr, SCALP_DPT_SELF_ADDR, SCALP_DPT_SELF_ADDR, SCALP_MPUGYR, mpu.data.gyro_x_hi, mpu.data.gyro_x_lo, mpu.data.gyro_y_hi, mpu.data.gyro_y_lo, mpu.data.gyro_z_hi, mpu.data.gyro_z_lo)
                                && scalp_dpt_tx(&mpu.interf, &fr));
...
void mpu_init(void)
{
        ...
        mpu.interf.cmde_mask = SCALP_DPT_CM(SCALP_TWIREAD)
                             | SCALP_DPT_CM(SCALP_TWIWRITE);
